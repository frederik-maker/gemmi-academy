...
The primary tutor is **Gemma 4 E2B-it running on-device through LiteRT-LM**, inside a Capacitor Android plugin. The model is the `gemma-4-E2B-it.litertlm` build the LiteRT community team publishes on Hugging Face — roughly 2 GB, int4-quantised, and publicly downloadable without a Hugging Face token. The APK ships without it. The student opens the app, taps "enable offline AI", the bundle downloads to `context.filesDir`, and from then on the tutor runs entirely on the phone — no keys, no API spend, no data leaving the device.
...
Roughly 1,943 hand-curated trilingual questions cover G1 through G3 across math, science, history, society, and English; another ~3,000 pulled from MMLU-Pro fill out the G4-G5 grade bands. The hand-written G1-G3 content lives in `src/data/` as plain JavaScript objects, one file per subject, while G4-G5 lazy-loads from `public/packs/` the first time a student opens that grade — there's no point parsing thousands of MMLU questions on cold start when most users never reach them. Each question carries its prompt and options in all three languages so the same lesson works no matter which UI language is active. A `forLangs` field hides lessons that only make sense in some languages — vocab-translation prompts like "What does 'Dog' mean?" appear for Kazakh and Russian students learning L2 English, but skip the English-UI student for whom the question has no answer.
...
**Main Track.** Gemmi is a shipped product, not a concept demo. The site is live at gemmi.ai, the APK is installable from the GitHub release, the curriculum is open, and the whole repo is public. Kids in rural Kazakhstan are the actual target audience — the kind of phones, networks, and language situations that ruin most ed-tech demos. So the app runs on entry-level Android, switches between Қазақша, Русский, and English mid-session, and stays usable when the connection drops. The tutor reads the student's real progress on every turn rather than improvising one, and accepts a photo of a notebook page when the student would rather show their work than describe it.

**Future of Education (Impact).** The brief asks for multi-tool agents that adapt to the individual and integrate cleanly with the way kids and educators already work. Gemmi's tutor has four tools wired into the student's live state and uses them implicitly — it doesn't ask "would you like me to pull up your stats?", it just does, and references "you missed two quadratics on Tuesday" the same way a tutor sitting next to the student would. The Ask Gemmi pill on every lesson question opens the chat already grounded in what the student is stuck on, so the conversation starts in context instead of from scratch.

**LiteRT (Special Technology).** The on-device path is the whole point. A Capacitor Android plugin pulls Gemma 4 E2B-it from the LiteRT community Hugging Face repo, stores the 2 GB `.litertlm` bundle in `context.filesDir`, and loads it through LiteRT-LM. From then on the same four-tool agent loop that runs in the cloud runs on the phone — tool calls, multi-turn replies, struggles-log context, all of it — with no network involved. The downloader is built around the assumption that the 2 GB transfer will fail mid-way several times: every attempt resumes through HTTP Range, retries on `SSLException` / `SocketTimeoutException` / mid-stream `IOException`, and SHA-256-verifies the bytes before swapping `.part` into place.
